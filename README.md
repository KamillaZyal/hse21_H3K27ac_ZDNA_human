...
- Используем R-библиотеку ChIPpeakAnno для ассоциаций пересееней (диапозон: -2000-3000):
  > Для обработки данных используем скрипт */src/ChIPpeakAnno.R*
 ```
...
# Итоги
- После приведения данных к версии генома hg19 было потеряно около 1% пиков и столько же было после фильтрации.
- При пересечении гистоновой метки и стр-ры ДНК было получено 13121 пиков (DeepZ содержит 19294 пика).
- Большая часть пиков после финальной обработки расположена на промоторах (около 85%), что подтверждаяет функцию гистонной метки, а так же особенности метода DeepZ (см. диаграмму пай-чарт для DeepZ - около 70%).
- Наиболее значимыми категориями после GO-анализ оказались основные метаболистические и биосинтаксические  процессы.
